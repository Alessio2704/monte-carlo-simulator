...
TEST_F(EngineErrorTests, ThrowsOnVectorSizeMismatch)
{
    ...
    {
        EXPECT_TRUE(std::string(e.what()).find("Vector size mismatch: element-wise operation requires vectors of the same length, but got sizes 2 and 3.") != std::string::npos);
    }
}
    ...
    {                                                                                                                                                                                                                                                                                  \
        ...
        EXPECT_THROW({ \
            try { \
                ...
                EXPECT_TRUE(std::string(e.what()).find(expected_error_msg) != std::string::npos); \
                throw; \
            } }, std::runtime_error);                                                                                                                                                                                                                                         \
    }

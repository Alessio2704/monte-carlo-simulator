...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "let a = if then 10 else 11",
            ErrorCode.SYNTAX_INVALID_CHARACTER,
            id="ce_missing_condition",
        ),
        pytest.param(
            "let a = if true then else 11",
            ErrorCode.SYNTAX_INVALID_CHARACTER,
            id="ce_missing_then_value",
        ),
        pytest.param(
            "let a = if true then 10 else",
            ErrorCode.SYNTAX_PARSING_ERROR,
            id="ce_missing_else_value",
        ),
        pytest.param(
            "let a = if x then if y then 1 else 2",
            ErrorCode.SYNTAX_INVALID_CHARACTER,
            id="ce_dangling_else_binds_to_inner_if",
        ),

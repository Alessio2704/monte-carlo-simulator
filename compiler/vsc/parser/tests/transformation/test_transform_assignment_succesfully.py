...
execution_func_call_nested_exp = get_execution_assignment(target="a", 
                                                          expression=get_function_call("add", args=[
                                                            get_function_call("multiply", args=[
                                                                get_function_call("Normal", args=[
                                                                    get_function_call("Pert", args=[get_number_literal(1), get_number_literal(2), get_number_literal(3)]),
...
execution_vec_elem_access_exp = get_execution_assignment(target="a", 
                                                         expression=get_function_call("add", args=[
                                                             get_element_access("my_vec", get_number_literal(0)),
                                                             get_number_literal(10)
                                                         ]))

execution_vec_elem_delete_code = "let a = my_vec[:1] + 10"
execution_vec_elem_delete_exp = get_execution_assignment(target="a", 
                                                         expression=get_function_call("add", args=[
                                                             get_delete_element("my_vec", get_number_literal(1)),
                                                             get_number_literal(10)
                                                         ]))
execution_comparison_and_precedence_code = "let a = x > 5 and y < 10"
execution_comparison_and_precedence_exp = get_execution_assignment(target="a",
...
variadic_exp = get_execution_assignment(target="a",
                                        expression=get_function_call(function="add", args=[
                                            get_number_literal(1),
...
variadic_nested_exp = get_execution_assignment(target="a",
    expression=get_function_call(function="add", args=[
        get_number_literal(1),
...
precedence_default_exp = get_execution_assignment(target="a",
                                                  expression=get_function_call("add", args=[
                                                      get_number_literal(10),
                                                      get_function_call("multiply", args=[
                                                          get_number_literal(11),
                                                          get_number_literal(22)
                                                      ])
                                                  ]))

precedence_modified_code = "let a = (10 + 11) * 22"
precedence_modified_exp = get_execution_assignment(target="a",
                                                  expression=get_function_call("multiply", args=[
                                                      get_function_call("add", args=[
                                                          get_number_literal(10),
                                                          get_number_literal(11)
                                                      ]),
                                                      get_number_literal(22),
...
conditional_basic_exp = get_conditional_assignment(target="a",
                                                   condition=get_boolean_literal(True),
                                                   then_expr=get_number_literal(4),
                                                   else_expr=get_number_literal(5))

conditional_cond_is_other_var_code = "let a = if b then 4 else 5"
conditional_cond_is_other_var_exp = get_conditional_assignment(target="a",
                                                   condition=get_identifier("b"),
                                                   then_expr=get_number_literal(4),
                                                   else_expr=get_number_literal(5))

conditional_returns_are_other_vars_code = "let a = if true then b else c"
conditional_returns_are_other_vars_exp = get_conditional_assignment(target="a",
                                                   condition=get_boolean_literal(True),
                                                   then_expr=get_identifier("b"),
                                                   else_expr=get_identifier("c"))

conditional_nested_else_code = "let a = if true then 1 else if false then 13 else 14"
conditional_nested_else_exp = get_conditional_assignment(target="a",
                                                   condition=get_boolean_literal(True),
                                                   then_expr=get_number_literal(1),
                                                   else_expr=get_conditional_expression(
                                                       condition=get_boolean_literal(False),
                                                       then_expr=get_number_literal(13),
                                                       else_expr=get_number_literal(14)))

conditional_then_is_func_call_code = "let a = if b then some_func(1) else 5"
conditional_then_is_func_call_exp = get_conditional_assignment(target="a",
                                                   condition=get_identifier("b"),
                                                   then_expr=get_function_call("some_func", args=[get_number_literal(1)]),
                                                   else_expr=get_number_literal(5))

conditional_cond_is_func_code = "let a = if some_func() then b else c"
conditional_cond_is_func_exp = get_conditional_assignment(
    target="a",
    condition=get_function_call("some_func", args=[]),
    then_expr=get_identifier("b"),
    else_expr=get_identifier("c")
)

conditional_as_func_arg_code = "let a = my_func(if x then 1 else 2)"
conditional_as_func_arg_exp = get_execution_assignment(target="a",
                                                       expression=get_function_call(
                                                           function="my_func", 
                                                           args=[
...
multi_assignment_basic_exp = get_multi_assignment(targets=[get_identifier("a"), get_identifier("b")],
                                            expression=get_function_call(function="some_func", args=[]))

multi_assignment_3_vars_code = "let a, b, c = some_func()"
multi_assignment_3_vars_exp = get_multi_assignment(targets=[get_identifier("a"), get_identifier("b"), get_identifier("c")],
                                            expression=get_function_call(function="some_func", args=[]))

multi_assignment_var_code = "let a, b = my_tuple"
multi_assignment_var_exp = get_multi_assignment(
    targets=[get_identifier("a"), get_identifier("b")],
    expression=get_identifier("my_tuple")
)

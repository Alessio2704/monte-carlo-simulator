...
basic_func_expected = get_function_def(name="my_func", return_type="scalar", body=[get_return_statement(get_number_literal(1))])

# Test spaces tabs and comments
space_tab_comments_code = "func my_func() -> scalar {#nfuybfbe \n \t \t \t \n return 1 }"
space_tab_comments_expected = get_function_def(name="my_func", return_type="scalar", body=[get_return_statement(get_number_literal(1))])
...
tuple_simple_expected = get_function_def(
    name="my_func",
    return_type="scalar",
    body=[get_return_statement(get_tuple_literal(items=[get_number_literal(1), get_number_literal(2)]))],
)
...
tuple_sophisticated_body_expected = get_function_def(
    ...
    body=[
        ...
        get_return_statement(get_tuple_literal(items=[get_identifier("x"), get_identifier("y")])),
    ],
)
...
body_complex_expected_1 = get_function_def(
    ...
    body=[
        get_literal_assignment(target="a", value=get_number_literal(10)),
        get_literal_assignment(
            target="b",
            value=get_vector_literal(
                [
        ...
        get_return_statement(
            get_function_call(
                ...
                args=[
                    get_function_call(
                        function="Pert",
                        args=[get_number_literal(1), get_number_literal(2), get_number_literal(3)],
                    ),
...
conditional_return_expected = get_function_def(
    name="a",
    body=[
        get_return_statement(
            returns=get_conditional_expression(
...
return_nested_math_expected = get_function_def(
    name="a",
    body=[
        get_return_statement(
            returns=get_function_call(
                function="divide",
                args=[
                    get_function_call(function="add", args=[get_identifier("x"), get_identifier("y")]),
                    get_identifier("z"),
                ],
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            body_assignment_and_return_code,
            body_assignment_and_return_expected,
            id="body_simple_assignment_and_return",
        ),
